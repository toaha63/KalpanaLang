...
class LanguageTranslator
{
    ...
    public static boolean willDebugMessageShowedForGenerics = !false;
    static
    {
        // BN -> BN
    ...
    private static synchronized String translateNonStringPart(String text, Map<String, String> translationMap)
    {
        ...
        for (String keyword : keywords)
        {
            ...
            if (translationMap == englishToBangla)
            {
                ...
                result = Pattern.compile("\\b" + Pattern.quote(keyword) + "\\b", Pattern.CASE_INSENSITIVE).matcher(result).replaceAll(replacement);
            }
            else
            {
    ...
    private static String processForLoopFunctionCalls(String code) {
        ...
        Pattern pattern = Pattern.compile(
            "লুপ\\s*\\(" + 
            "\\s*((পূর্ণসংখ্যা|ভগ্নাংশ|বাক্য|বুলিয়ান)\\s+)?(\\w+)\\s*=\\s*(\\w+)\\s*\\([^)]*\\)\\s*;"
        );
...
record Function(Token name, Token returnType, List<Token> parameters, 
                List<Stmt> body, boolean isTemporary, Environment environment) implements Stmt
{
    
    // Constructor for regular functions (without environment)
    public Function(Token name, Token returnType, List<Token> parameters, 
                   List<Stmt> body, boolean isTemporary)
        {
        this(name, returnType, parameters, body, isTemporary, new Environment());
    }
    ...
    {
        // Create a new environment that encloses the function's environment
        ...
        return new Function(this.name, this.returnType, this.parameters, 
                           this.body, this.isTemporary, boundEnvironment);
    }
...
// New AST nodes for access modifiers
...
class Parser
{
    ...
    private Stmt declaration()
    {
        try
        {
            if (match(TokenType.INTEGER, TokenType.FLOAT, TokenType.STRING, TokenType.BOOLEAN, TokenType.OBJECT, TokenType.ALL))
            {
    ...
    private Stmt classMember()
    {
        ...
        if (match(TokenType.INTEGER, TokenType.FLOAT, TokenType.STRING, TokenType.BOOLEAN, TokenType.ALL))
        {
    ...
    private Stmt functionDeclaration()
    {
        ...
        else if(match(TokenType.INTEGER, TokenType.FLOAT, TokenType.STRING, TokenType.BOOLEAN,TokenType.INTEGER_ARRAY, TokenType.FLOAT_ARRAY,TokenType.STRING_ARRAY, TokenType.BOOLEAN_ARRAY, TokenType.ALL))
        {
            returnType = previous();
        }
        else
        ...
        if (!check(TokenType.RIGHT_PAREN))
        {
            do
           {
                ...
                if (match(TokenType.INTEGER, TokenType.FLOAT, TokenType.STRING, TokenType.BOOLEAN,TokenType.INTEGER_ARRAY, TokenType.FLOAT_ARRAY,TokenType.STRING_ARRAY, TokenType.BOOLEAN_ARRAY, TokenType.ALL))
                {
                    Token type = previous();
                    Token param = consume(TokenType.IDENTIFIER, "Expect parameter name.");
                    param.type = type.type;
    ...
    private Expr primary()
    {
        ...
        } else if (match(TokenType.IDENTIFIER))
        {
            ...
            else if (match(TokenType.LEFT_BRACKET))
            {
                ...
                expr = new ArrayAccess(new Variable(name), index);
            } else
            {
            ...
            while (true)
            {
                if (match(TokenType.DOT))
                {
                    ...
                    if (match(TokenType.LEFT_PAREN))
                    {
                        ...
                        Token paren = consume(TokenType.RIGHT_PAREN, "Expect ')' after arguments.");
                        expr = new MethodCallExpr(expr, name, arguments);
                    } else
                    {
...
class Environment
{
    ...
    private final Map<String, TokenType> arrayTypes = Collections.synchronizedMap(new LinkedHashMap<>());
    private final Map<String, Boolean> unlimitedArrays = Collections.synchronizedMap(new LinkedHashMap<>());
...
class ClassInstance
{
    ...
    public static Object convertToType(Object value, TokenType expectedType)
    {
        ...
        // Handle scalar types (existing code)
        switch (expectedType)
        {
            case INTEGER:
                if (value instanceof BigDecimal)
...
class Interpreter implements ExprVisitor<Object>, StmtVisitor<Void>
{
    ...
    {
        // This method now only handles file opening
        if (stmt.operation() != FileOperation.OPEN)
        {
            throw new RuntimeException("File statement can only be used for opening files");
        }
    ...
    public Object visitObjectExpr(ObjectExpr expr)
    {
        ...
        ClassInstance instance = new ClassInstance(classDef, initialFieldValues);
    ...
    public Object visitCallExpr(Call expr)
    {
        ...
        {
            throw new RuntimeException("Expected " + function.parameters().size() + " arguments but got " + expr.arguments().size() + ".");
        }
        ...
        {
            // Return type checking with automatic conversion
            ...
            if (function.returnType().type != TokenType.VOID)
            {
                ...
                Object finalValue = returnValue.value;
                switch (function.returnType().type)
                {
                    case ALL:
                        break;
                    case INTEGER:
                        if (!(finalValue instanceof BigDecimal))
                        {
                            throw new RuntimeException("Function must return an integer");
                        }
                        return ((BigDecimal)finalValue).setScale(0, RoundingMode.DOWN);
    
                    case FLOAT:
                        if (finalValue instanceof BigDecimal)
                        {
                            // If returning an integer value but function expects float, add .0
                            if (((BigDecimal)finalValue).scale() <= 0)
                            {
                                return new BigDecimal(finalValue.toString() + ".0");
                            }
                            return finalValue;
                        }
                ...
                // In the return type checking switch statement
                case BOOLEAN:
                    if (returnValue.value instanceof BigDecimal)
                    {
                        // C-style boolean conversion for return values
                        return !((BigDecimal)returnValue.value).equals(BigDecimal.ZERO);
                    }
                    ...
                    case INTEGER_ARRAY:
                        ...
                        for (Object element : (Object[])finalValue)
                        {
                            if (!(element instanceof BigDecimal))
                            {
    ...
    public Void visitVarStmt(Var stmt)
    {
        ...
        if (stmt.initializer() != null)
        {
            ...
            // Handle case where boolean is assigned a boolean literal
            else if (stmt.name().type == TokenType.BOOLEAN && !(value instanceof Boolean))
            {
    ...
    public Void visitArrayStmt(ArrayStmt stmt)
    {
        ...
        // Initialize array if not already initialized by আর্গুমেন্ট() or direct array assignment
        if (array == null)
        {
            switch (stmt.type())
            {
        ...
        if (!isArgumentFunction && !(stmt.initialValues().size() == 1 && array instanceof Object[] && array.length == size))
        {
        ...
        case ASSIGN:
            ...
            else if (expr.left() instanceof DotExpr)
            {
                ...
                if (object instanceof ClassInstance)
                {
                    ...
                    // Set the field value on the class instance
                    instance.set(dotExpr.name(), right);
                    return right;
                }
    ...
    public Object visitArrayAssignmentExpr(ArrayAssignment expr)
    {
        ...
        // Handle unlimited array size - resize if needed (only for non-class arrays)
        if (index > arr.length && !(expr.array() instanceof ClassSelfExpr))
        {
            ...
            // Update the array in environment (only for non-class arrays)
        ...
        else if (index > arr.length && expr.array() instanceof ClassSelfExpr)
        {
            throw new RuntimeException("Array index out of bounds for class field");
        }
    ...
    protected String stringify(Object object)
    {
        ...
        if (object instanceof BigDecimal)
        {
            ...
            boolean isFloat = bd.scale() > 0 || (object instanceof Variable && ((Variable)object).name().type == TokenType.FLOAT);
...
public class Main
{
            ...
            for (i = 0; i < args.length; i++)
            {
                ...
                if (arg.startsWith("-"))
                {
                    ...
                    case "-a":
                        ...
                        System.out.println(RED + "NOTE: This KalpanaLang  special edition also is released in memory of the \"Milestone School and College\" plane crash tragedy. Decicated to all our younger brothers & sisters of Milestone School & College ❤️" + RESET);
                        System.gc();
                        System.exit(0);
    ...
    private static String formatAsJson(Duration elapsed, long memBefore, long memAfter)
    {
        ...
        return String.format("{\"time\": %.6f, \"unit\": \"s\", \"memory_mb\": %.2f}",seconds, memoryMB);
    }

    private static void printUsage()
    {
        System.out.println(BLUE + "Usage: kalpana <file.kls> [options]" + RESET);
